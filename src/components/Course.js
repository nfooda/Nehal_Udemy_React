...
function Course({ course }) {
    ...
    return (
        <div className={CourseStyle.course}>
            <img src={course.image} alt={course.title} className={CourseStyle.courseImg} />
            <h3 className={CourseStyle.textHeadingSM}> {course.title}</h3>
            <p className={`${CourseStyle.instructor} ${CourseStyle.textSM}`}> {Instructors()}</p>
            <div className={CourseStyle.rate}>
                <span className={`${CourseStyle.rateNo}`}>{rating}</span>
                <Rating name="half-rating-read" defaultValue={rating} precision={0.5} readOnly />

            </div>
            <h3 className={`${CourseStyle.textHeadingSM} ${CourseStyle.price}`}>{`E£ ${course.price}`}</h3>
        </div>
